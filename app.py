...
# ---------- PAGE CONFIG ----------
st.set_page_config(page_title="RapidTest.ai Demo", layout="wide", page_icon="📈", initial_sidebar_state="expanded")
st.markdown("""
    <style>
    [data-testid="stHeader"] {background-color: #1f3b70;}
    .css-1d391kg, .stApp {background-color: #ffffff;}
    h1, h2, h3, h4, h5 {color:#1f3b70;}
    </style>
...
if submitted:
    ...
    with st.spinner("Building charts..."):
        ...
        ax1.set_title("Monthly Recurring Revenue"); ax1.set_xlabel("Month"); ax1.set_ylabel("MRR ($)")
        ...
        ax2.set_title("Cumulative Revenue"); ax2.set_xlabel("Month"); ax2.set_ylabel("Revenue ($)")
    ...
    with st.spinner("Generating Investor Deck..."):
        ...
        def make_pdf(path, title, desc, metrics):
            ...
            c.setFont("Helvetica",12); c.drawString(50,h-90,desc)
            c.setFillColor(colors.black); y=h-120; c.setFont("Helvetica",11)
            for m in metrics:
                c.drawString(60,y,m); y-=15
            c.save()
        ...
        make_pdf("partner_sales_sheet.pdf","Scale Your SaaS Clients with RapidTest.ai",
                 ...
                 [f"Growth Rate: {growth:.1f}%", f"Churn: {churn:.1f}%",
                  f"12-Month ARR: ${arr_est:,.0f}", f"Engagement Uplift: +{uplift:,.1f}%"])
        # ZIP
        buffer = BytesIO()
        with ZipFile(buffer, "w") as z:
            z.write("investor_brief.pdf")
            z.write("partner_sales_sheet.pdf")
        st.download_button(label="📁 Download Investor Deck (ZIP)",
